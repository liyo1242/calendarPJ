...
Calendar.prototype.resorted = function() {
    for(var dalf = 0; dalf < 6; dalf++){
        $(`.days .r-${dalf} label .dayEvent`).each(function(){ // days event =>
            // Sort by time
            var gandalfSort = [];
            $(this).children().each(function() { // time title fulldayflag
                // save eventTime and id
                gandalfSort.push({id: $(this).attr('class'), time: $(this).find('.timePresent').html(), full: $(this).find('.fullday').length})
            });

            gandalfSort = gandalfSort.sort(function (a, b) {
             return a.time > b.time ? 1 : -1;
            });
            // console.log(gandalfSort);

            // assign the order
            for(var gandalfOrder = 0; gandalfOrder < gandalfSort.length; gandalfOrder++){
                $(this).find(`.${gandalfSort[gandalfOrder].id}`).css('order',gandalfOrder + 1);
                $(this).find(`.${gandalfSort[gandalfOrder].id}`).css('top',`${(gandalfOrder*33)}%`);


                if(gandalfOrder >= 3){
                    $(this).find(`.${gandalfSort[gandalfOrder].id}`).css('display',"none");
                }else {
                    $(this).find(`.${gandalfSort[gandalfOrder].id}`).css('display',"inline");
                }
            }
        })
    }
...
Calendar.prototype.remastered = function() {
    ...
    for(var dalf = 0; dalf < 6; dalf++){
        ...
        $(`.days .r-${dalf} label .dayEvent>div .fullday`).each(function(){
            gandalfBox.push($(this).parent().attr('class'))
        })

        holyGandalfBox = Array.from(new Set(gandalfBox)); // holyGandalfBox = [...(new Set(gandalfBox))]

        gandalfCount = gandalfBox.reduce((obj, item) => {
            obj[item] = obj[item] ? obj[item] + 1 : 1;
            return obj;
        }, {});

        $.each(gandalfCount, function(index, value) {
            ...
            $(`.days .r-${dalf} .${index} .timePresent`).css('display','none');
            $(`.days .r-${dalf} .${index} .titlePresent`).css('display','none');
            //
            $(`.days .r-${dalf} .${index}:eq(0)`).css('display','inline');
            // $(`.days .r-${dalf} .${index}:eq(-1)`).css('display','inline');
            $(`.days .r-${dalf} .${index} .timePresent:eq(0)`).css('display','inline');
            $(`.days .r-${dalf} .${index} .titlePresent:eq(0)`).css('display','inline');
            // $(`.days .r-${dalf} .${index} .timePresent:eq(0)`).html(gandalfPencil);
            // $(`.days .r-${dalf} .${index} .titlePresent:eq(0)`).html(gandalfPencil2);
            $(`.days .r-${dalf} .${index}:eq(0)`).css("width",`${value}00%`);
        });
...
Organizer.prototype.list = function(a) {
    ...
    for (var d, b = document.createElement("UL"), c = 0; c < a.length; c++) {
        ...
        h.appendChild(document.createTextNode(a[c].title)),d.appendChild(f), d.appendChild(h), b.appendChild(d);
    }

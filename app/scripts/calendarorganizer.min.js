...
Organizer.prototype = {
    ...
    Organizer.prototype.list = function(data) {
        ...
        for (var d, b = document.createElement("UL"), c = 0; c < data.length; c++) {
            ...
            h.appendChild(document.createTextNode(data[c].title)), d.appendChild(f), d.appendChild(h), d.appendChild(lineTransfer), listArray.push(d);
        }
        listArray.sort((a, b) => {
          if (Date.parse(moment().get('year') + "/" + moment().get('month') + "/" + moment().get('date') + " " + a.childNodes[0].innerText.slice(0,5)).valueOf() < Date.parse(moment().get('year') + "/" + moment().get('month') + "/" + moment().get('date') + " " + b.childNodes[0].innerText.slice(0,5)).valueOf()) {
            return -1;
          }
          if (Date.parse(moment().get('year') + "/" + moment().get('month') + "/" + moment().get('date') + " " + a.childNodes[0].innerText.slice(0,5)).valueOf() > Date.parse(moment().get('year') + "/" + moment().get('month') + "/" + moment().get('date') + " " + b.childNodes[0].innerText.slice(0,5)).valueOf()) {
            return 1;

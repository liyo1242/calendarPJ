...
Organizer.prototype.list = function(data) {
    ...
    for (var d, b = document.createElement("UL"), c = 0; c < data.length; c++) {
        ...
        var eventStr = `${sendData.title}%0a開始時間:${sendData.time}日 ${sendData.startTime}%0a結束時間:${sendData.time}日 ${sendData.endTime}%0a地點:${sendData.location}%0a活動內容:${sendData.text}`;
        var holyEventStr = `${sendData.title}開始時間:${sendData.time}日 ${sendData.startTime}結束時間:${sendData.time}日 ${sendData.endTime}地點:${sendData.location}活動內容:${sendData.text}`
...
Organizer.prototype.setupBlock = function(a, b, c) {
    ...
    document.getElementById(d.id + "-day-" + a).onclick = function() {
        ...
        $body.animate({
            scrollTop: $('#organizerContaine').offset()
        }, 1000);
...
Organizer.prototype.setOnClickListener = function(Identifier, b, c) {
    var cid = this.calendar.id,
        id = this.id;
    b = b || function() {}, c = c || function() {};
    var g = this;
    switch (Identifier) {
        case "days-blocks":
            ...
            break;
        case "day-slider":
            document.getElementById(id + "-day-back").onclick = function() {
                g.back("day"), b()
            }, document.getElementById(id + "-day-next").onclick = function() {
                g.next("day"), c()
            };
            break;
        case "month-slider":
            document.getElementById(cid + "-month-back").onclick = function() {
                g.back("month"), b()
            }, document.getElementById(cid + "-month-next").onclick = function() {
                g.next("month"), c()
            };
            break;
        case "goToToday":

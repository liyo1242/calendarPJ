...
function Calendar(id, size, labelSettings, colors, func) {
    ...
    console.log("fucfuck label = " + this.label);
    for (var l = 0; 7 > l; l++)
        this.labels.push(this.label[l]);
    console.log("fucfuck labelaaaaasss = " + this.labels);

    this.date = new Date;
    console.log("fucfuck Date = " + this.date);
    this.today = new Date;
    console.log("fucfuck today = " + this.today);
    this.draw();
    this.update();
    ...
    for (var d = [], f = ["year", "month", "labels", "days"], g = 0; g < f.length; g++)
        if (d[g] = document.createElement("DIV"), d[g].className = f[g], "days" != f[g])
            if ("month" == f[g]) {
                ...
                var week = ["日", "一", "二", "三", "四", "五", "六"]; // dropdown list
            ...
            if(eventData[fuck.getFullYear()][fuck.getMonth() + 1 ] != undefined){
                ...
                if(dayEventLine != undefined){
                    for (var Roundtable = document.createElement("DIV"), cc = 0; cc < dayEventLine.length; cc++) {
                        ...
                        Knights.appendChild(document.createTextNode(dayEventLine[cc].title));
                        if(cc > 2){
...
Organizer.prototype.list = function(a) {
    ...
    for (var d, b = document.createElement("UL"), c = 0; c < a.length; c++) {
        ...
        h.appendChild(document.createTextNode(a[c].title)),dbtn.appendChild(document.createTextNode("delete")),ubtn.appendChild(document.createTextNode("update")), d.appendChild(ubtn) ,d.appendChild(f), d.appendChild(h), d.appendChild(dbtn) , b.appendChild(d);
    }

...
Organizer.prototype = {
    ...
    Organizer.prototype.list = function(data) {
        ...
        listArray.sort((a, b) => {
            console.log('sorting');
          if (Date.parse(moment().get('year') + "/" + moment().get('month') + "/" + moment().get('date') + " " + a.childNodes[0].innerText).valueOf() < Date.parse(moment().get('year') + "/" + moment().get('month') + "/" + moment().get('date') + " " + b.childNodes[0].innerText).valueOf()) {
            return -1;
          }
          if (Date.parse(moment().get('year') + "/" + moment().get('month') + "/" + moment().get('date') + " " + a.childNodes[0].innerText).valueOf() > Date.parse(moment().get('year') + "/" + moment().get('month') + "/" + moment().get('date') + " " + b.childNodes[0].innerText).valueOf()) {
            return 1;
